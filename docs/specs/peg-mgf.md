---
title: "PEG-MGF — Technical Specification"
description: "Factory-direct PEG-MGF wholesale. MW ~11,500 Da. ≥99% HPLC. PEGylated IGF-1Ec — satellite cell activation. COA included."
---

# PEG-MGF

## Identification

| Property | Value |
|---|---|
| **CAS Number** | — (IGF-1Ec splice variant, PEGylated) |
| **Molecular Formula** | — |
| **Molecular Weight** | ~11,500 Da (PEG-protein conjugate) |
| **Sequence** | IGF-1Ec E-domain peptide (24-aa insert), PEGylated |
| **Amino Acid Count** | ~94 + PEG moiety |
| **Appearance** | White to off-white lyophilized powder |
| **Purity Standard** | ≥99% by HPLC |
| **Solubility** | Soluble in sterile water or PBS |
| **Storage** | 2–8°C, desiccated, protect from light |

**Category:** Muscle Growth
  
**Catalog SKU:** PSH-PEGMGF-1MG

## Research Background

PEG-MGF (PEGylated Mechano Growth Factor) is a modified form of IGF-1Ec — the splice variant of IGF-1 preferentially expressed in skeletal muscle in response to mechanical load. MGF differs from systemic IGF-1 by a 24-amino acid C-terminal E-domain insert that confers distinct receptor binding and specifically activates muscle satellite (stem) cells. PEGylation extends the peptide's half-life from minutes to ~24 hours, making it viable for chronic muscle regeneration research.

### Key Mechanisms

| Mechanism | Details |
|---|---|
| **IGF-1Ec Signaling** | E-domain peptide binds distinct receptor from IGF-1 → satellite cell-specific activation |
| **PEG Conjugation** | Polyethylene glycol chain extends half-life from minutes to ~24 hours |
| **Myoblast Proliferation** | Satellite cell → myoblast → myotube fusion → new myofiber nuclei |

## Research Focus Areas

### Satellite Cell Activation
Muscle stem cell proliferation and differentiation

### Muscle Regeneration
Post-damage myofiber repair and hypertrophy models

### Sarcopenia Research
Age-related muscle stem cell depletion countermeasures

### Exercise Recovery
Mechanical load-induced MGF expression simulation

## Available Configurations

| Configuration | Content | Best For |
|---|---|---|
| **Standard Kit** | 1 mg × 10 vials | Satellite cell research; evaluation |
| **Bulk Kit** | 2 mg × 10 vials | Muscle regeneration protocols; repeat orders |
| **Custom** | Custom mg × custom vials | OEM; private-label |

## Tiered Wholesale Pricing

| Volume | Discount | For |
|---|---|---|
| 1 kit | List price | Evaluation; competitive analysis |
| 5+ kits | 10% off | Initial portfolio expansion |
| 20+ kits | 20% off | Mid-size distributors |
| 50+ kits | 30% off | Regional distributors |

## Quality Specifications

| Parameter | Method | Criterion |
|---|---|---|
| Purity | HPLC at 214 nm, C18 column | ≥99.0% peak area |
| Identity (MW) | MALDI-TOF MS | ~11,500 ± 500 Da |
| Appearance | Visual inspection | White to off-white powder |
| PEGylation | SDS-PAGE | Band shift confirmed |
| Endotoxin | LAL assay | ≤0.5 EU/mg |

## Frequently Asked Questions

### Q1: What is Mechano Growth Factor (MGF)?

MGF is the IGF-1Ec splice variant expressed locally in skeletal muscle following mechanical load or damage. It contains a 24-amino acid E-domain that targets muscle satellite cells specifically. The 'mechano' prefix refers to its induction by mechanical stress.

### Q2: Why is PEGylation necessary for MGF?

Native MGF has a very short half-life (minutes). PEGylation increases hydrodynamic radius, reduces renal clearance, and extends half-life to ~24 hours. Without PEGylation, MGF would not be viable for most research protocols.

### Q3: How does MGF differ from standard IGF-1?

(1) MGF is a local splice variant expressed in muscle, not liver-derived; (2) the E-domain insert confers satellite cell receptor specificity; (3) MGF's primary role is stem cell activation/myoblast proliferation, while systemic IGF-1 primarily drives protein synthesis in mature myofibers.

### Q4: What research applications use PEG-MGF?

Satellite cell (muscle stem cell) biology, muscle regeneration/repair following injury models, sarcopenia research, and exercise physiology studies exploring the MGF response to mechanical loading.

### Q5: What QC is unique to PEG-MGF?

Beyond standard HPLC and MS: SDS-PAGE confirms PEGylation band shift; LAL endotoxin testing; NMR or colorimetric PEG verification. All in batch-specific COA.

## Key Research References

| Reference | PMID | Key Finding |
|---|---|---|
| Yang S & Goldspink G. "Different roles of the IGF-I Ec peptide (MGF) and mature IGF-I in myoblast proliferation and differentiation." *FEBS Lett*. 2002;522(1-3):156–160. | 12095637 | MGF (IGF-1Ec) E-domain peptide specifically activates satellite cell (myoblast) proliferation while mature IGF-1 drives differentiation; distinct receptor targets |
| Hill M & Goldspink G. "Expression and splicing of the insulin-like growth factor gene in rodent muscle is associated with muscle satellite (stem) cell activation following local tissue damage." *J Physiol*. 2003;549(Pt 2):409–418. | 12692175 | Mechanical loading induces MGF splice variant expression locally in muscle; rapid satellite cell activation response to muscle damage |

## Stability & Storage

| Condition | Degradation Profile |
|---|---|
| **Native Half-Life** | Native (non-PEGylated) MGF has a very short half-life (minutes). PEGylation is essential for research viability |
| **PEG Conjugate Stability** | The polyethylene glycol moiety dramatically extends half-life from minutes to ~24 hours and provides steric protection against proteolysis |
| **Lyophilized Storage** | 24 months at 2–8°C in desiccated environment, protect from light |
| **Reconstituted Solution** | Use within 24 h at 2–8°C. PEG-MGF is a ~11.5 kDa PEG-protein conjugate — handle gently, never vortex. Aliquot and freeze at −20°C |
| **PEGylation Verification** | SDS-PAGE band shift confirmation in every batch; PEG-specific colorimetric or NMR assay for bulk orders |

## Related Products

| Product | Link |
|---|---|
| **HGH (Somatropin)** | [hgh.md](hgh.md) — 191-aa human growth hormone |
| **CJC-1295 (DAC)** | [cjc-1295-dac.md](cjc-1295-dac.md) — Long-acting GHRH analogue |
| **Ipamorelin** | [ipamorelin.md](ipamorelin.md) — Selective GHS-R1a agonist |
| **AOD-9604 (hGH Fragment)** | [aod-9604.md](aod-9604.md) — Lipolytic hGH fragment 177-191 |

## Related Resources

- [All Product Specifications](index.md)
- [COA & Purity Analysis Guide](../quality/coa-purity-analysis.md)
- [Quality Control Framework](../quality/index.md)
- [Main Website → PEG-MGF](https://peptidesourcehub.net/products/peg-mgf/)
